...
class drawer_model extends CI_Model
{
    ...
    public function getStartDrawer($shiftID){
        $query = $this->db->query("SELECT select coalesce(initDrawer,0)  FROM shift where shiftID= '".$shiftID."' ");
        if ($query->num_rows() > 0) {
            return $query->result_array();
        } else {
            return 0;
    ...
    public function getDrawerDetails($shiftID){
        $query = $this->db->query("select
         shift.initDrawer ,  drawerLub ,  drawerAccess ,  drawerWash ,
         drawerReturn , drawerFuel , drawerFuelDebit,totalDebits,
         drawerPayC from shift   
        left join 
        (select coalesce(sum(amount-rest),0) as drawerLub,shiftID as shiftID1 FROM invoice WHERE type = 'lub' and isSupply= 0 
        and shiftID = '".$shiftID."' ) as tableSumLub on shift.shiftID = tableSumLub.shiftID1
        left join 
        (select coalesce(sum(amount-rest),0) as drawerAccess,shiftID as shiftID2 FROM invoice WHERE type = 'access' and isSupply= 0 
        and shiftID = '".$shiftID."' ) as tableSumAcc on shift.shiftID = tableSumAcc.shiftID2
        left join 
        (select coalesce(sum(amount-rest),0) as drawerWash,shiftID as shiftID3 FROM invoice WHERE type = 'wash' and isSupply= 0 
        and shiftID = '".$shiftID."' ) as tableSumWash on shift.shiftID = tableSumWash.shiftID3
        left join 
        (select coalesce(sum(amount),0) as drawerReturn,shiftID as shiftID4 FROM invoice WHERE type = 'return' and isSupply= 0 
        and shiftID = '".$shiftID."' ) as tableSumReturn on shift.shiftID = tableSumReturn.shiftID4
        left join 
        (select coalesce(sum(amount),0) as drawerPayC,shiftID as shiftID5 FROM invoice WHERE type = 'payC' and isSupply= 0 
        and shiftID = '".$shiftID."'  ) as tableSumPayC on shift.shiftID = tableSumPayC.shiftID5
        left join 
        (select coalesce(sum(amount),0) as drawerFuel,shiftID as shiftID6 FROM invoice WHERE  
        type in ('Diesel G','Diesel R','95','98') and isSupply= 0 
        and shiftID = '".$shiftID."'  ) as tableSumFuel on shift.shiftID = tableSumFuel.shiftID6
        left join 
        (select coalesce(sum(rest),0) as drawerFuelDebit,shiftID as shiftID7 FROM invoice WHERE  
        type in ('98_d','95_d','dieselG_d','dieselR_d') and isSupply= 0 
        and shiftID = '".$shiftID."') as tableSumFuel_d on shift.shiftID = tableSumFuel_d.shiftID7
        left join 
        (select coalesce(sum(rest),0) as totalDebits,shiftID as shiftID8 FROM invoice WHERE  
        isSupply= 0 
        and shiftID = '".$shiftID."') as tableSumTotalDeits on shift.shiftID = tableSumTotalDeits.shiftID8
        where shift.shiftID = '".$shiftID."' ");
         if ($query->num_rows() > 0) {
            return $query->result_array();
        } else {
